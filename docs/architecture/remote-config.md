  ...
  "primary": {
    ...
    "transport": {
      ...
      "params": {
        ...
        "path": "/opaque-websocket-path",
        "server_name": "cover.example",
        "host_header": "cover.example",
        "fingerprint": "chrome"
      }
    }
  },
...
- **не больше трёх нод.** `primary` и до двух reserve. Ни одного поля, из которого можно судить о полном размере флота
...
- **транспорт — конверт, а не набор полей верхнего уровня.** Всё специфичное для WebSocket/TLS или запасного REALITY живёт внутри `transport.params`; схема плана знает только строку `kind`
...
```json
{ "supported_transports": ["vless-ws-tls"], "app_version": 3 }
```
...
Действующий основной transport — `vless-ws-tls`. Механизм согласования существует с первого дня по причине, изложенной в [evolution.md](evolution.md): второй transport не должен требовать одновременного обновления всех установок.
