...
Канал закрывает единственный сценарий, недоступный остальным трём: **первая установка**, когда у клиента ещё нет ни одного плана, а `https-direct` и `reality-edge` заблокированы. У существующих пользователей эту роль играет канал `tunnel`.
