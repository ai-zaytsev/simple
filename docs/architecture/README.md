...
Техническая архитектура Android VPN. Документы определяют границы компонентов и ограничения, которым обязана подчиняться действующая реализация.

Фактическое размещение, провайдеры, домены, состояние, панели, действия при отказе и масштабирование описаны только в [операционной инструкции Business Owner](../business-owner-operations.md). Архитектурные документы не являются вторым инвентарём.
...
| [infrastructure.md](infrastructure.md) | Действующая логическая топология и технические инварианты | Разворачиваете что-либо |
| [mvp-topology.md](mvp-topology.md) | Исторические sizing-решения и границы бюджета | Проверяете, почему MVP размещён компактно |
...
Архитектурные решения остаются в [decisions.md](decisions.md), внешние prerequisites — в [prerequisites.md](prerequisites.md), принятый операционный долг — в [tech-debt.md](../tech-debt.md). Текущий состав инфраструктуры и действующие лимиты из этих документов не выводятся: их источник истины — BO-инструкция и указанные в ней live-панели.
