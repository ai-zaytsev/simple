...
FREE и VIP не различаются качеством инфраструктуры. И те и другие ноды — расходный ресурс с одинаковым жизненным циклом, см. [node-lifecycle.md](node-lifecycle.md). Различия исчерпываются четырьмя параметрами.
...
VIP включается только в транзакции обработки канонически подтверждённого платежа. Возврат браузера в приложение не является подтверждением. `entitlement_applied_at` и блокировка строки платежа делают повторный webhook безопасным: один платёж добавляет срок один раз. По истечении `vip_expires_at` аккаунт возвращается в FREE, внешние credentials отзываются, а лимит FREE немедленно применяется к установкам приложения.
