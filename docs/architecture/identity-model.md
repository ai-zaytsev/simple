...
1. **Аналитическое хранилище не содержит ни email, ни `account_id`, ни `device_id`, ни `vpn_credential_id`.** Единственный пользовательский ключ в аналитике — `analytics_id`.
...
- **отслеживание конкретного человека между эпохами невозможно.** Business Owner подтвердил, что этого и не требуется: достаточно агрегированного retention. Он считается механизмом когорт, описанным ниже.

## Когорты И Агрегированный Retention

К каждому событию аналитики Control Plane добавляет `cohort` — огрублённый признак, вычисляемый из месяца регистрации аккаунта:

```
cohort = год-месяц регистрации аккаунта
```

Это позволяет считать retention без какой-либо связи между эпохами:

```
retention(cohort C, эпоха E) = uniq(analytics_id где cohort = C в эпоху E) / размер когорты C
```

Ни один запрос не требует знать, что `analytics_id` из эпохи 1 и `analytics_id` из эпохи 2 принадлежат одному человеку. Считаются мощности множеств, а не пересечения по людям.

Требования к признаку:

- гранулярность не тоньше календарного месяца: более точная дата регистрации при малом числе пользователей превращается в идентификатор
- когорты с числом участников ниже порога схлопываются в общую
- `cohort` — единственный признак, привязанный к аккаунту и переживающий смену эпохи; он низкокардинальный и не индивидуальный

Цена: `analytics_id` несёт в себе месяц регистрации, что незначительно повышает связуемость записей внутри эпохи. Размен принят: это одно низкокардинальное поле против полноценной когортной аналитики.

Чего этот механизм не даёт и не должен давать: индивидуальных траекторий, ответа на вопрос «вернулся ли конкретный пользователь», построения профиля поведения во времени.
...
flowchart LR
    ...
    ING -->|"credential_id → account_id → analytics_id"| AN[("Аналитика<br/>только analytics_id")]
...
Почему нельзя писать `vpn_credential_id` в аналитику напрямую: credential живёт дольше эпохи и является стабильным псевдонимом. Его наличие в аналитике сделало бы ротацию `analytics_id` бессмысленной.
...
Проверка этого правила должна стать автоматической на стадии реализации: схема аналитики не содержит соответствующих колонок, а тест проверяет отсутствие запрещённых полей в событиях ingest.
