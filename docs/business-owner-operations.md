...
| Версия приложения | latest `0.1.0 (1)`, minimum `1`, channel artifacts `1` | Первая официальная APK опубликована 2026-09-01; SHA-256 `e99ce3d9…`, сертификат `38dfe1cd…` |
| Официальный сайт | Работает через Cloudflare: `/` = 200, `/healthz` = 204; live deploy `33506736236`: `No changes`, `0 added / 0 changed / 0 destroyed`, responsive Android/Samsung guide опубликован, HTML перепроверяется браузером | `site-1` и CI-доступ восстановлены без создания или замены Droplet; firewall после deploy снова web-only; первая официальная APK опубликована и отдаётся с `/releases.json` и `/download/latest.apk` |
...
| Скорость **трафика через VPN** | 10 Мбит/с | Без предела |
| Месячная квота трафика | Не применяется | Не применяется |

Ограничение скорости применяется **на ноде**, к трафику, который через неё идёт. Российские адреса при `russia_direct` уходят с устройства напрямую, ноды не касаются и под ограничение не попадают: зарубежный спидтест на FREE покажет 10 Мбит/с, российский — домашний канал. Оба числа верные и меряют разное. Подробнее — [entitlement-model.md](architecture/entitlement-model.md#чего-лимит-не-касается).

Практическое следствие для цены: **пользователю с преимущественно российским трафиком FREE ощущается почти безлимитным**, и «без ограничения скорости» покупает для него меньше, чем читается по таблице.

> **Меняешь лимит — правь сайт.**
>
> Числа на странице загрузки (`sites/official/index.html`, раздел «Бесплатный режим») **ни с чем не связаны** и связаны не будут: это решение от 2026-09-02, риск принят осознанно. Сайт статический и на другом хосте, спросить лимит ему не у кого.
>
> Значит `update tier_limits set speed_mbit = ...` меняет то, что применяется, а сайт продолжает обещать прежнее — молча, ничего не ломая. На другом конце покупатель, выбравший продукт по этому числу.
>
> Порядок: изменить лимит → поправить число в `index.html` (оба места: `data-free-speed` и текст рядом) → выкатить сайт `Deploy APK Site`. Проверка `check-site-says-the-real-limits.sh` не даст числу разойтись внутри репозитория, но про живое изменение она не знает — знать должен ты.
