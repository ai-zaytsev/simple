...
| --- | --- | --- |
| `support@simple-vpn.download` в Cloudflare Email Routing | Этот адрес вшит в каждую установку **0.1.0**. С 2026-09-21 минимальная версия — **2**: связавшись с Core, 0.1.0 показывает неотменяемый диалог обновления и до экрана «Помощь» не пускает | Через пару недель, когда устройства успеют связаться с Core |
...
1. ~~Решить про 0.1.0.~~ **Сделано 2026-09-21:** минимальная версия поднята до 2. В коде опубликованной 0.1.0 проверено, что диалог обязательного обновления не закрывается, кнопки «позже» нет, VPN не запускается, а адрес загрузки проверяется только на `https` и непустой хост — белого списка доменов нет, так что новую ссылку она принимает.
2. Убрать правило Email Routing для `support@simple-vpn.download` — **не сразу**. Требование обновления 0.1.0 узнаёт только от Core; устройство, которое до Core не достучалось, покажет старый интерфейс целиком, вместе со старым адресом. Пара недель даёт устройствам время связаться.
3. Убрать `mail.simple-vpn.download` из Brevo.
4. Удалить записи зоны в Cloudflare или дать домену истечь.

Пока не прошло это время, **зону не удалять**: письма в поддержку с устройств, не узнавших о требовании, начнут отбиваться, а узнать об этом будет неоткуда.
...
| Домен ноды заблокирован | Device checks плохие, **Core checks хорошие** (вердикт `likely blocked`) | Создать ноду с новым IP/доменом, затем вывести старую |
| Нода не отвечает никому | Device checks плохие и **Core checks плохие** (вердикт `unreachable`) | Это не блокировка, машину не менять. `Node Inspect` — при `nginx: failed` или `port 443: NO LISTENER` поднять `Update Node`. Случалось дважды: 30.08.2026 и 23.09.2026 |
