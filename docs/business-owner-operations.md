...
| Официальный сайт | Работает: `/` = 200, `/healthz` = 204; read-only state check `33500134659`: `recorded_site=true`, `adoption=false`, `No changes` | `site-1` восстановлен power-cycle без создания или замены Droplet и после восстановления штатно записан в Terraform state; первый официальный APK ещё не опубликован |
