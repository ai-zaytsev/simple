...
- Channel material всегда относится к текущему `latest`: продвижение общей версии очищает ссылки предыдущей, а остальные каналы прикрепляются к тому же `versionCode`. Старый APK не может быть предложен как новая версия.
...
- Повтор публикации после временного сбоя синхронизации Core не перезаписывает immutable APK и штатно завершает Core latest.
