...
- `evolution.md` — инварианты, обеспечивающие последующее резервирование Core и добавление второго транспорта
- `decisions.md` — ADR по спорным вопросам
- `prerequisites.md` — внешние ресурсы, которые нужно запросить у Business Owner

## Решения Business Owner, Полученные В Ходе Стадии

Приняты и отражены в документах:

1. Достаточно агрегированного retention; отслеживание конкретного пользователя месяцами не требуется.
2. До публичного запуска обязателен резервный способ первичного получения рабочей конфигурации при блокировке основных каналов.
3. Один Core и один VPN-транспорт принимаются как временные ограничения MVP при условии, что архитектура не мешает последующему резервированию Core и добавлению альтернативного транспорта.
4. Оплата откладывается за пределы MVP.
5. FREE и VIP — одинаково расходуемый инфраструктурный ресурс. VIP отличается только квотой трафика, числом устройств и возможностью выгрузить конфигурацию для сторонних клиентов в виде QR, ссылки или JSON. для следующих стадий
